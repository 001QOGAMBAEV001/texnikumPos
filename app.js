...
const token = '6437581102:AAEC-f0gOsI-mIGyK0dH2vnVBBIAYIgXwm0';
...
const talimTurlari = [
    'Kunduzgi / Дневное',
    'Kechki / Вечернее',
    'Sirtqi (faqat Markazlashtirish va post operatori uchun) / Заочное (только для Централизация и пост оператор)',
    'Dual (ishlab yurgan odamlar uchun) / Дуальное (для работающих людей)'
];
...
async function handleUserResponse(chatId, text) {
    ...
    if (step === 7) {
        ...
        let availableTalimTurlari;

        if (selectedYonalish.includes('Markazlashtirish va post operatori')) {
            availableTalimTurlari = talimTurlari;
        } else {
            availableTalimTurlari = talimTurlari.filter(t => !t.includes('Sirtqi') && !t.includes('Dual'));
        }
